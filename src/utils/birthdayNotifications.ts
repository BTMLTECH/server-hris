...
export async function seedMonthlyBirthdays(company: ICompany) {
...
export async function runBirthdayNotifications(company: ICompany) {
  const today = new Date();
...
export async function updateReturnedLeaves(company:ICompany) {
  const now = new Date();

  // Find all leaves that ended but are not marked as returned
  const leavesToUpdate = await LeaveRequest.find({
    status: 'Approved',
    isActive: true,
    returned: false,
    endDate: { $lt: now },
  ...
  for (const leave of leavesToUpdate) {
    ...
    if (leave.user) {
      await sendNotification({
        ...
        message: `Your ${leave.type} leave has ended.`,
        emailSubject: 'Leave Completed',
        emailTemplate: 'leave-ended.ejs',
        emailData: {
      ...
      // emitToUser(leave.user._id as Types.ObjectId, 'leave:returned', {
      //   leaveId: leave._id,
      //   type: leave.type,
      // });
    }
