...
//     type: [{ type: String, enum: ['reliever', 'teamlead', 'hr', 'md'] }],
//     default: function () {
//       if (!this.relievers || this.relievers.length === 0) {
//         return ['hr', 'md']; // teamlead leave
//       }
//       const relieverCount = Math.min(Math.max(this.relievers?.length || 2, 2), 3);
//       return Array(relieverCount).fill('reliever').concat(['teamlead', 'hr']);
//     },
...
//       validator: function (v: string[]) {
//         if (!Array.isArray(v)) return false;
//         // If there are no relievers, it's a teamlead leave
//         if (!this.relievers || this.relievers.length === 0) {
//           return v.length === 2 && v[0] === 'hr' && v[1] === 'md';
//         }

//         // For normal staff leaves, enforce 2–3 relievers then teamlead → hr
//         const relieversCount = v.filter((r) => r === 'reliever').length;
//         const endsCorrectly = v[v.length - 2] === 'teamlead' && v[v.length - 1] === 'hr';
//         return (relieversCount === 2 || relieversCount === 3) && endsCorrectly;
//       },
//       message:
//         'Review flow must start with 2 or 3 relievers, then include teamlead → hr as final approvers, or hr → md for teamlead leave',
//     },
...
const LeaveRequestSchema = new Schema<ILeaveRequest>({
  ...
  // },
    ...
    default: function () {
      ...
      const relieverCount = Math.min(Math.max(this.relievers?.length || 2, 2), 3);
      return Array(relieverCount).fill('reliever').concat(['teamlead', 'hr', 'md']);
    },
  ...
  validator: function (v: string[]) {
    ...
    // Teamlead leave → hr → md
    if (!this.relievers || this.relievers.length === 0) {
      return v.length === 2 && v[0] === 'hr' && v[1] === 'md';
    }

    // Staff leave → relievers → teamlead → hr → md
    const relieversCount = v.filter(r => r === 'reliever').length;

    const endsCorrectly =
      v[v.length - 3] === 'teamlead' &&
      v[v.length - 2] === 'hr' &&
      v[v.length - 1] === 'md';

    return (
      (relieversCount === 2 || relieversCount === 3) &&
      endsCorrectly
    );
  },
  message:
    'Review flow must be relievers → teamlead → hr → md, or hr → md for teamlead leave',
},
