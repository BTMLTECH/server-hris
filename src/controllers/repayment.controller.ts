...
export const makeLoanRepayment = asyncHandler(
  async (
    ...
    if (amountPaid > remainingBalance) {
      throw new ErrorResponse(
        `Overpayment not allowed. Remaining balance is ₦${remainingBalance.toLocaleString()}`,
        400,
      );
    }

    // Step 2: Ensure 1 payment per month
    const lastPayment = await LoanRepayment.findOne({ loanId, user: userId }).sort({
      paymentDate: -1,
    });
    ...
    if (lastPayment) {
      ...
      if (new Date() < nextAllowedPayment) {
        throw new ErrorResponse(
          `Only one payment allowed per month. Next allowed: ${nextAllowedPayment.toDateString()}`,
          400,
        );
      }
    }
...
export const getRepaymentHistory = asyncHandler(
  async (req: TypedRequest, res: TypedResponse<ILoanRepayment[]>) => {
